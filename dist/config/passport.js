...
function JSTStrat(payload, done) {
...
function localStrat(username, password, done) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const user = yield user_model_1.default.findOne({ email: username }).exec();
            if (!user)
                return done(null, false, { message: routeHelpers_1.messages.UNKNOWN_EMAIL });
            if (user && (yield user_model_2.comparePassword(password, user.password)))
                return done(null, user);
            else
                return done(null, false, { message: routeHelpers_1.messages.UNKOWN_USER_PASSWORD });
        }
        catch (error) {
